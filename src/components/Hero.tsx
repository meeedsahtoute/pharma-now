...
export const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <section className="relative overflow-hidden bg-slate-900 dark:bg-slate-950 text-white pt-10 pb-12 sm:pt-14 sm:pb-16 border-b border-slate-800">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative z-10">
        ...
        {/* Main Headline */}
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-white mb-4 leading-[1.15] whitespace-pre-line">
          {t.heroTitle}
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-bold border border-slate-700 transition active:scale-95"
          >
        ...
        <div className="relative max-w-2xl mx-auto mb-6">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-950/80 border border-slate-700/80 text-white placeholder-slate-400 text-sm sm:text-base font-medium focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 shadow-2xl transition"
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-center flex-wrap gap-2 text-xs">
          <span className="text-slate-400 font-medium me-1">{t.popularCities}</span>
          {moroccanCities.map((cityName) => (
            <button
              key={cityName}
              onClick={() => onSelectCityDirect(cityName)}
              className="px-3 py-1 rounded-xl bg-slate-800/80 hover:bg-emerald-600 hover:text-white border border-slate-700 text-slate-300 font-semibold transition active:scale-95 flex items-center gap-1"
            >
