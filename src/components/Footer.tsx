...
export const Footer: React.FC<FooterProps> = ({ onTabChange, onCityClick }) => {
  ...
  return (
    <footer className="bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-t border-slate-200 dark:border-slate-800 pt-12 pb-8 mt-16 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pb-8 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="md:col-span-5 flex flex-col gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-extrabold text-lg tracking-tight text-slate-900 dark:text-white font-sans">
                PHARMA<span className="text-emerald-600 dark:text-emerald-400 ms-1">NOW</span>
              </span>
            </div>
            ...
            {/* Privacy Notice */}
            <div className="mt-2 p-3 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 shadow-sm">
          ...
          <div className="md:col-span-4 flex flex-col gap-2">
            ...
            <ul className="space-y-2 text-xs font-semibold text-slate-600 dark:text-slate-400">
              <li>
                <button onClick={() => onTabChange('find')} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition">
                  {t.navFindPharmacy}
                </button>
              </li>
              <li>
                <button onClick={() => onTabChange('duty')} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition">
                  {t.navOnDuty}
                </button>
              </li>
              <li>
                <button onClick={onCityClick} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition">
                  {t.navCities}
                </button>
              </li>
              <li>
                <button onClick={() => onTabChange('how')} className="hover:text-emerald-600 dark:hover:text-emerald-400 transition">
                  {t.navHowItWorks}
                </button>
              </li>
            </ul>
          ...
          <div className="md:col-span-3 flex flex-col gap-3">
            <h4 className="font-bold text-xs uppercase tracking-wider text-slate-900 dark:text-white mb-1">
              Language / اللغة
            </h4>
