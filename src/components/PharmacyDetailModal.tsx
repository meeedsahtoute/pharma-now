      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                statusInfo?.badgeColor === 'amber' ? 'bg-amber-400/20 text-amber-300 border border-amber-500/40' :
                statusInfo?.badgeColor === 'emerald' ? 'bg-emerald-400/20 text-emerald-300 border border-emerald-500/40' :
                statusInfo?.badgeColor === 'rose' ? 'bg-rose-400/20 text-rose-300 border border-rose-500/40' :
                'bg-slate-800 text-slate-400 border border-slate-700'
              }`}>
                {statusInfo?.status === 'on_duty' && <Moon className="w-3.5 h-3.5 text-amber-400 animate-pulse" />}
                {statusInfo?.statusLabel}
              </span>

              <span className="text-xs font-mono font-bold text-emerald-400 bg-slate-800 px-2.5 py-1 rounded-full border border-slate-700">
                {formatDistance(pharmacy.distanceMeters)}
              </span>
            </div>

            <h2 className="text-xl font-black text-slate-100">{pharmacy.name}</h2>
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          {pharmacy.dutyInfo.dutyNote && (
            <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/30 text-amber-200">
              <div className="flex items-center gap-2 font-bold text-amber-300 text-sm mb-1">
                <Moon className="w-4 h-4 text-amber-400" />
                <span>Night Bell & Emergency Access</span>
              </div>
              <p className="text-xs text-amber-200/90 leading-relaxed">{pharmacy.dutyInfo.dutyNote}</p>
            </div>
          )}
          ...
          <div>
            ...
            {pharmacy.openingHours?.isConflicting ? (
              ...
              <div className="bg-slate-950/60 border border-slate-800 rounded-2xl overflow-hidden divide-y divide-slate-800/60">
                {days.map(({ key, label }) => {
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <a
                ...
                className="flex items-center justify-center gap-1.5 p-3 rounded-xl bg-slate-800 hover:bg-slate-750 text-slate-200 border border-slate-700 text-xs font-bold transition"
              >
        ...
        <div className="p-4 bg-slate-950 border-t border-slate-800 flex items-center justify-between gap-3">
          {pharmacy.phone ? (
            <a
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-sm shadow-lg shadow-emerald-500/20 active:scale-95 transition"
            >
            ...
            <button
              disabled
              className="flex-1 py-3 rounded-2xl bg-slate-800 text-slate-500 font-bold text-xs border border-slate-700 cursor-not-allowed"
            >
